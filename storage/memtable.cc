...
void MemTable::insert(const char* key, size_t key_size, const char* value, size_t value_size, ValueType value_type)
{
    ...
    writeToBuffer(buffer + offset,
        key_size, std::make_pair(key, key_size),
        value_size, std::make_pair(value, value_size));
